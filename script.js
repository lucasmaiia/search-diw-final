function loadProfile() {
    ...
    xhr.onload = function() {
        ...
        if (data.bio == null) {
            data.bio = `O usuário não possui biografia.`;
        }
        ...
        <span id="profileFollowers"><i class="fas fa-users"></i> ${data.followers} seguidores</span>
        <span id="profileFollowing"><i class="fas fa-users"></i> ${data.following} seguindo</span>
    ...
    xhr.open("GET", `https://api.github.com/users/lucasmaiia`);
    xhr.send();
}
...
function repositoryProfile() {
    ...
    xhr2.onload = function() {
        ...
        for (let i = 0; i < data2.length; i++) {
            ...
            <a id="repositoryLink" href="${put.html_url}" target="_blank"><button id="repositoryButton" class="btn"><i class="far fa-folder-open"></i> Repositório</button></a>
            <a id="repositoryWeb" href="https://${put.owner.login}.github.io/${put.name}" target="_blank"><button id="siteButton" class="btn"><i class="far fa-window-restore""></i> Website</button></a>
            </div>
            </div>
            </div>`;
        }
    ...
    xhr2.open("GET", `https://api.github.com/users/lucasmaiia/repos`);
    xhr2.send();
}
